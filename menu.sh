...
if [[ -z "$NET" ]]; then NET="eth0"; fi
vnstat_safe(){ if command -v vnstat >/dev/null 2>&1; then vnstat -i "$NET" "$@"; else return 1; fi }
dtoday="$(vnstat_safe | grep "today" | awk '{print $2" "substr ($3, 1, 1)}' || echo "-")"
utoday="$(vnstat_safe | grep "today" | awk '{print $5" "substr ($6, 1, 1)}' || echo "-")"
ttoday="$(vnstat_safe | grep "today" | awk '{print $8" "substr ($9, 1, 1)}' || echo "-")"
#Download/Upload yesterday
dyest="$(vnstat_safe | grep "yesterday" | awk '{print $2" "substr ($3, 1, 1)}' || echo "-")"
uyest="$(vnstat_safe | grep "yesterday" | awk '{print $5" "substr ($6, 1, 1)}' || echo "-")"
tyest="$(vnstat_safe | grep "yesterday" | awk '{print $8" "substr ($9, 1, 1)}' || echo "-")"
#Download/Upload current month
dmon="$(vnstat_safe -m | grep "`date +"%b '%y"`" | awk '{print $3" "substr ($4, 1, 1)}' || echo "-")"
umon="$(vnstat_safe -m | grep "`date +"%b '%y"`" | awk '{print $6" "substr ($7, 1, 1)}' || echo "-")"
tmon="$(vnstat_safe -m | grep "`date +"%b '%y"`" | awk '{print $9" "substr ($10, 1, 1)}' || echo "-")"
clear
# OS Uptime
...
echo -e   "  \e[$number (18)\e[m \e[$below INSTALL BOT (Telegram)\e[m        \e[$number (19)\e[m \e[$below SETTING BOT\e[m"
echo -e   " \e[$line════════════════════════════════════════════════════════════\e[m"
